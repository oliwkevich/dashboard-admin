...
export const CellAction = ({ data }: { data: ColorColumn }) => {
  ...
  const onCopy = () => {
    ...
    toast.success("ID колір було скопійовано!");
  };
  ...
  const onRemove = async () => {
    ...
    try {
      ...
      toast.success(
        `Ви успішно видалили колір ${data?.name.toUpperCase()}, сторінка будет перезавантажена!`
      );
      router.refresh();
    } catch (error) {
  ...
  return (
    <>
      <AlertModal
        ...
        onConfirm={onRemove}
        title="Ви точно хочете видалити колір?"
      />
      <DropdownMenu>
