...
export const CellAction = ({ data }: { data: SizeColumn }) => {
  ...
  const onCopy = () => {
    ...
    toast.success("ID розмір було скопійовано!");
  };
  ...
  const onRemove = async () => {
    ...
    try {
      ...
      toast.success(
        `Ви успішно видалили розмір ${data?.name.toUpperCase()}, сторінка будет перезавантажена!`
      );
      router.refresh();
    } catch (error) {
  ...
  return (
    <>
      <AlertModal
        ...
        onConfirm={onRemove}
        title="Ви точно хочете видалити розмір?"
      />
      <DropdownMenu>
