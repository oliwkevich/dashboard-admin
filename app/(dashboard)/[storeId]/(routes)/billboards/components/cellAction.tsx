...
export const CellAction = ({ data }: { data: BillboardColumn }) => {
  ...
  const onCopy = () => {
    ...
    toast.success("ID дошки було скопійовано!");
  };
  ...
  const onRemove = async () => {
    ...
    try {
      ...
      toast.success(
        `Ви успішно видалили дошку ${data?.label.toUpperCase()}, сторінка будет перезавантажена!`
      );
      router.refresh();
    } catch (error) {
  ...
  return (
    <>
      <AlertModal
        ...
        onConfirm={onRemove}
        title="Ви точно хочете видалити дошку?"
      />
      <DropdownMenu>
